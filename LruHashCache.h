...
template<typename key_t, typename value_t, typename HashFun = HashFunctor<key_t> >
class NormalHashMapCache
{
public:
    typedef HashFun hash_func;
    typedef hash_map<key_t, value_t, hash_func> LruDataMap;

    NormalHashMapCache()
    {
    }

    NormalHashMapCache(size_t max_size, bool getUpdate = true)
    {
    }
    ...
    void drain(int m)
    {
        ...
        else
        {
            while (m-- && size() > 0)
            {
                _cache_items_map.erase(_cache_items_map.begin());
            }
        }
    }
    ...
    void swap(NormalHashMapCache& newLruCache)
    {
        _cache_items_map.swap(newLruCache._cache_items_map);
    }
    ...
    const LruDataMap& getHashMapRef()
    {
        return _cache_items_map;
    }
